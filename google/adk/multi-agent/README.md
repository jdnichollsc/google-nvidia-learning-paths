...
```mermaid
graph LR
    root_agent --> sub_agent1
    root_agent --> sub_agent2
    sub_agent1 --> sub_agent_1_1
    sub_agent1 --> sub_agent_1_2
    sub_agent_1_2 --> sub_agent_1_2_1
    sub_agent_1_2 --> sub_agent_1_2_2
    sub_agent2 --> sub_agent_2_1
    sub_agent2 --> sub_agent_2_2
```
...
1. Paste the following commands into the Cloud Shell Terminal to copy code files from a Cloud Storage bucket:
...
Click **Check my progress** to verify the objective. *Explore transfers between parent, sub-agent, and peer agents.*

## Task 3. Use session state to store and retrieve specific information

Each conversation in ADK is contained within a `Session` that all agents involved in the conversation can access. Each session has a **session state** dictionary: a simple Python dictionary you can use to store values that you would like to highlight and how they are accessed.
...
Click **Check my progress** to verify the objective. *Use session state to store and retrieve specific information.*

> **Note:** Instead of saving small pieces of information, if you would like to store your agent's entire text response to state, you can use the `output_key` parameter when creating an agent.
...
```mermaid
graph LR
    greeter["[greeter]<br/>requests a historical<br/>subject"]
    user_input["'Ada Lovelace'"]

    subgraph film_concept_team["SequentialAgent: film_concept_team"]
        subgraph writers_room["LoopAgent: writers_room"]
            wikipedia["⚡ [wikipedia tool]"]
            researcher["[researcher]<br/>researches using<br/>Wikipedia"]
            screenwriter["[screenwriter]<br/>writes a draft"]
            critic["[critic]<br/>offers feedback<br/>on the draft"]

            researcher -->|uses| wikipedia
            researcher --> screenwriter
            screenwriter --> critic
            critic -.->|"decides whether to<br/>iterate or exit<br/>the writing loop"| researcher
        end

        subgraph preproduction_team["ParallelAgent: preproduction_team"]
            box_office["[box_office_researcher]<br/>estimates box office results<br/>based on similar films"]
            casting["[casting_agent]<br/>generates ideas for<br/>character casting"]
        end

        file_writer["[file_writer]<br/>Aggregates output<br/>and writes a file"]

        writers_room --> preproduction_team
...
Click **Check my progress** to verify the objective. *Begin building a multi-agent system with a SequentialAgent.*
...
Click **Check my progress** to verify the objective. *Add a LoopAgent for iterative work.*
...
In this lab, you will add some supplemental reports — some research on potential box office performance and some initial ideas on casting based on actors who have starred in similar films — to enhance the pitch for your new film.
...
graph LR
    ...
    subgraph film_concept_team["SequentialAgent: film_concept_team"]
        ...
        subgraph preproduction_team["ParallelAgent: preproduction_team"]
            box_office_researcher["[box_office_researcher]<br/>analyzes potential<br/>box office"]
            casting_agent["[casting_agent]<br/>suggests actors<br/>for the film"]
        end
...
- The **greeter** will remain the same.
- The **film_concept_team** `SequentialAgent` will now consist of:
